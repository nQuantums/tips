...
class ExperienceBuffer(object):
	...
	def construct_nstep_transition(self, data):
		if self.idx == -1: #  Episode ended at the very first step in this n-step transition
			return
		key = f'{self.id}_{self.n_step_seq_num}'
		...
		if self.idx + 1 < self.capacity:
			...
			for i in range(blen - 1):
				...
				for k in range(i + 1, blen):
					Gamma *= g
					R += Gamma * l1b[k].R
				l1b[i] = Transition(l1b[i].S, l1b[i].A, R, Gamma, l1b[i].q)
...
class Actor(mp.Process):
	...
	def epsilon_greedy_Q(self, qS_t):
		if random.random() >= self.epsilon:
			...
			return random.randint(0, len(qS_t) - 1)
	...
	def run(self):
		...
		for t in range(self.T):
			...
			if self.local_experience_buffer.size >= n_step_transition_batch_size:
				...
				# while self.global_replay_queue.qsize():
				# 	time.sleep(0.5)
...
if __name__ == "__main__":
	...
	params = {"local_experience_buffer_capacity": 10, "epsilon": 0.4, "alpha": 7, "gamma": 0.99, "num_actors": 2, "n_step_transition_batch_size": 5, "Q_network_sync_freq": 10, "num_steps": 3, "T": 50000}
