...
def dqn_batchnorm_prelu(state_shape, action_dim, show_plot_name=None, hidden_size=32):
	...
	with model as m:
		...
		c = c.conv2d(state_shape[0], hidden_size, 8, 4).batchnorm2d(hidden_size).prelu()
		if show_plot_name:
			c = c.plot_img(show_plot_name, 0, ax_cnv1)
		c = c.conv2d(hidden_size, hidden_size * 2, 4, 2).batchnorm2d(hidden_size * 2).prelu()
		if show_plot_name:
			c = c.plot_img(show_plot_name, 0, ax_cnv2)
		c = c.conv2d(hidden_size * 2, hidden_size * 2, 3, 1).batchnorm2d(hidden_size * 2).prelu()
		if show_plot_name:
			c = c.plot_img(show_plot_name, 0, ax_cnv3)
		f = c.flatten()
