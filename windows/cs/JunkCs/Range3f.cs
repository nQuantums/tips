...
namespace Jk {
	...
	[Serializable]
	public struct Range3f : IJsonable {
		...
		public Range3f(vector position) {
			Min = position;
			Max = position;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3f(vector min, vector max) {
			Min = min;
			Max = max;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3f(vector min, vector max, bool normalize) {
			if (normalize) {
		...
		public Range3f(IEnumerable<vector> positions) {
		...
		public Range3f(IEnumerable<volume> volumes) {
			vector min = vector.MaxValue, max = vector.MinValue;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3f(Range3i v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3f(Range3d v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		...
		public bool Intersects(volume range) {
			...
			if (Max.Z < range.Min.Z || range.Max.Z < Min.Z) return false;
			return true;
		}
