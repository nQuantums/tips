...
namespace Jk {
	...
	[Serializable]
	public struct Range3d : IJsonable {
		...
		public Range3d(vector position) {
			Min = position;
			Max = position;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3d(vector min, vector max) {
			Min = min;
			Max = max;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3d(vector min, vector max, bool normalize) {
			if (normalize) {
		...
		public Range3d(IEnumerable<vector> positions) {
		...
		public Range3d(IEnumerable<volume> volumes) {
			vector min = vector.MaxValue, max = vector.MinValue;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3d(Range3i v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range3d(Range3f v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
