...
namespace Jk {
	...
	[Serializable]
	public struct Range2d : IJsonable {
		...
		public Range2d(vector position) {
			Min = position;
			Max = position;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range2d(vector min, vector max) {
			Min = min;
			Max = max;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range2d(vector min, vector max, bool normalize) {
			if (normalize) {
		...
		public Range2d(IEnumerable<vector> positions) {
		...
		public Range2d(IEnumerable<volume> volumes) {
			vector min = vector.MaxValue, max = vector.MinValue;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range2d(Range2i v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range2d(Range2f v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
