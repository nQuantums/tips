...
namespace Jk {
	...
	public static class Bool2DUtil<Material> where Material : class {
		#region 内部クラス
		/// <summary>
		...
		public class Loop {
			...
			/// </summary>
			public List<SharedSegment> Segments;

			public Loop() {
				this.Vertices = new List<Vertex>();
			}

			public Loop(List<Vertex> vertices) {
			...
			public bool Reduce(Dictionary<int, bool[]> segmentEnablesDic, element threshould, Func<vector, vector> tf, List<Vertex> workVertices, List<Vertex> reducedVertices) {
				...
				if (tf != null) {
					...
					workVertices.Clear();
					if (workVertices.Capacity < nvertices)
						workVertices.Capacity = nvertices;
				...
				// 頂点数減らす
				return Bool2DUtil<Material>.Reduce(
					segmentEnablesDic,
					vertices,
					this.Segments,
			...
			public Loop Clone() {
				...
				l.Vertices = new List<Vertex>(l.Vertices);
				l.Segments = new List<SharedSegment>(l.Segments);
				return l;
			}
		}
		...
		public class Polygon {
			...
			/// </summary>
			public Polygon(List<Loop> loops) {
				this.Loops = loops;
			}
			...
			public Polygon(IEnumerable<Loop> loops) {
				this.Loops = new List<Loop>(loops);
			}
			...
			public Polygon(int thisPolygonId, polbool pb, polbool.EPolygon epolygon) {
				...
				for (int iloop = 0, nloops = eloops.Count; iloop < nloops; iloop++) {
					var edges = eloops[iloop].Edges;
					var vertices = new List<Vertex>(edges.Count);
					for (int iedge = 0, nedges = edges.Count; iedge < nedges; iedge++) {
						vertices.Add(new Vertex(edges[iedge]));
					}
			...
			public polbool.Polygon ToBooleanInput(Dictionary<int, bool[]> segmentEnablesDic, Func<vector, vector> tf, Material faceMat, Material edgeMat, element threshould) {
				var srcLoops = this.Loops;
				var dstLoops = new List<polbool.VLoop>();
				...
				var workVertices = new List<Vertex>();
				var verticesReduced = false;
				for (int iloop = 0, nloops = srcLoops.Count; iloop < nloops; iloop++) {
#if BOOL2D_LOG
				using (Dbg.NewFrame("Lop", iloop + 1))
#endif
					{
						var srcLoop = srcLoops[iloop];

						// まず元の頂点数を減らす
						var reducedVertices = new List<Vertex>(srcLoop.Vertices.Count);
						if (srcLoop.Reduce(segmentEnablesDic, threshould, tf, workVertices, reducedVertices))
							verticesReduced = true;

						// ブーリアン演算用のループを作成する
						var dstLoop = new polbool.VLoop(
							new List<polbool.Vertex>(from v in reducedVertices select new polbool.Vertex(v.Position)),
							new List<object>(from v in reducedVertices select (object)v.ToEdgeAtr(edgeMaterial)));
						dstLoop.Volume = new volume(from v in dstLoop.Vertices select v.Position);
			...
			public Polygon Clone() {
				...
				p.Loops = new List<Loop>(p.Loops);
				var loops = p.Loops;
				for (int i = 0, n = loops.Count; i < n; i++) {
					loops[i] = loops[i].Clone();
				}
				return p;
			}
		}
		...
		public struct SharedSegment {
			...
			public override string ToString() {
				return string.Concat(this.Id, ": ", this.Start, " - ", this.End, " ", this.ForceTri);
			}
		...
		public struct IO {
			...
			/// ブーリアン演算入力
			/// </summary>
			public List<polbool.Polygon> Input;
			...
			/// ブーリアン演算結果
			/// </summary>
			public List<polbool.EPolygon> Output;
			...
			public IO(Func<vector, vector> tf, Material faceMat, Material outerMat, List<Polygon> polygons, element threshould, bool toInput) {
#if BOOL2D_LOG
			using (Dbg.NewFrame("Io"))
#endif
				{
					...
					var input = new List<polbool.Polygon>(polygons.Count);
#if BOOL2D_LOG
				using (0 <= threshould ? Dbg.NewFrameIncrement("Reduce") : Dbg.NullFrame())
...
#endif
							{
								input.Add(polygons[i].ToBooleanInput(segmentEnablesDic, tf, faceMat, outerMat, threshould));
							}
						}
					...
					if (toInput) {
						...
						for (int ipolygon = output.Count - 1; ipolygon != -1; ipolygon--) {
							var epolygon = output[ipolygon];
							SetSrcGroupPolygon(epolygon);
							epolygon.UserData = pb.Groups[epolygon.GroupIndex][epolygon.PolygonIndex].UserData;
						}
			...
			public IO(List<polbool.Polygon> input) {
				this.Input = input;
				this.Pb = null;
				this.Output = null;
				this.EdgeFilters = null;
			...
			public IO(polbool pb, EdgeFilters edgeFilters, List<polbool.EPolygon> output) {
				// 輪郭に対応するエッジに属性を設定する
				SetOutlineAtr(pb);
				...
				Dbg.StackWrite(output, "Before.csv");
					...
					using (Dbg.NewFrame("Pol", ipolygon + 1))
#endif
						{
							...
							var reducedEdges = new List<polbool.EDir>();
							var reducedELoops = new List<polbool.ELoop>();
							var verticesReduced = false;
							for (int iloop = 0, nloops = eloops.Count; iloop < nloops; iloop++) {
#if BOOL2D_LOG
							using (Dbg.NewFrame("Lop", iloop + 1))
#endif
								{
									...
									if (Reduce(segmentEnablesDic, eloop.Edges, eloop.UserData as List<SharedSegment>, project, threshould, reducedEdges))
										verticesReduced = true;
									reducedELoops.Add(new polbool.ELoop(new List<polbool.EDir>(reducedEdges)));
									eloop.UserData = null;
								}
							}
			...
			using (0 <= threshould ? Dbg.NewFrameIncrement("Reduce") : Dbg.NullFrame())
				...
				Dbg.StackWrite(output, "Before.csv");
					...
					var npolygons = output.Count;
					var input = new List<polbool.Polygon>(npolygons);
					var segmentEnablesDic = new Dictionary<int, bool[]>();
					...
					using (Dbg.NewFrame("Pol", ipolygon + 1))
#endif
						{
							...
							if (0 <= threshould) {
								...
								eloops = reducedELoops;
							}

							// エッジループ配列から頂点ループ配列を作成する
							var vloops = new List<polbool.VLoop>();
							foreach (var eloop in eloops) {
								var vloop = new polbool.VLoop(
									new List<polbool.Vertex>(from edge in eloop.Edges select new polbool.Vertex(edge.From.Position)),
									new List<object>(from edge in eloop.Edges select FindAtr(inputGroups, edge.Edge) as object));
							...
							polygon.UserData = pb.Groups[groupIndex][polygonIndex].UserData;
							if (verticesReduced)
								polygon.UserValue |= EPolygonFlags.VerticesReduced;
			...
			/// <summary>
			/// 入力または出力値を Polygon 配列に変換する
			/// </summary>
			/// <returns>Polygon 配列</returns>
			public List<Polygon> ToPolygons() {
				...
				// 出力値を Polygon に変換する
				var polygons = new List<Polygon>(npolygons);
				for (int ipolygon = 0; ipolygon < npolygons; ipolygon++) {
					var epolygon = output[ipolygon];
					var polygon = new Polygon(ipolygon + 1, pb, epolygon);
					var eloops = output[ipolygon].Loops;
					var loops = polygon.Loops;

					// セグメントを取得する
					for (int iloop = eloops.Count - 1; iloop != -1; iloop--) {
						loops[iloop].Segments = eloops[iloop].UserData as List<SharedSegment>;
					}
		...
		public static IO Or(List<Polygon> polygons) {
			var last = polygons.Count - 1;
			if (last < 0)
				return new IO();

			// 演算入力形式に変換する
			var ios = new IO[polygons.Count];
			for (int i = ios.Length - 1; i != -1; i--) {
				var pols = new List<Polygon>();
				pols.Add(polygons[i]);
				ios[i] = new IO(null, null, null, pols, -1, true);
			}
		...
		/// <returns>実際に頂点数が削減されたら true が返る</returns>
		public static bool Reduce<T>(Dictionary<int, bool[]> segmentEnablesDic, List<T> vertices, List<SharedSegment> segments, Func<T, vector> project, element threshould, List<T> reducedVertices) {
			var nvertices = vertices.Count;
			...
			// 凹凸サイズが無効なら頂点減らさない
			if (threshould < 0f) {
				reducedVertices.AddRange(vertices);
				return false;
			}

			// セグメントが１つ以下ならば全体を１つのセグメントとして頂点数を減らす
			if (segments == null || segments.Count <= 1) {
				var segmentEnables = new bool[vertices.Count];
				EnableVertices(vertices, 0, vertices.Count, segmentEnables, project, threshould, false);
				ExtractEnabledVertices(vertices, segmentEnables, reducedVertices);
				return vertices.Count != reducedVertices.Count;
			}

			// セグメント別に頂点数減らし処理をしていく
			var enables = new bool[nvertices];
			...
			ExtractEnabledVertices(vertices, enables, reducedVertices);
		...
		static void SetSrcGroupPolygon(polbool.EPolygon epolygon) {
			...
			if (srcGroupIndex == -1)
				throw new System.ApplicationException("Group not found from edges."); // TODO: 通常あり得ないのにここに来ることがある
		...
		static void SetOutlineAtr(polbool pb) {
			var groups = pb.Groups;
		...
		public static void SetSegments(List<polbool.EPolygon> epolygons) {
#if BOOL2D_LOG
		using(Dbg.NewFrameIncrement("SetSeg"))
#endif
			{
				var npolygons = epolygons.Count;

				// １つでも既に設定済みなら処理しない
				for (int ipolygon = npolygons - 1; ipolygon != -1; ipolygon--) {
					if ((epolygons[ipolygon].UserValue & EPolygonFlags.SegmentsSet) != 0)
						return;
				}
			...
			Dbg.StackWrite(EPolygonsDebug(epolygons), "AfterValue.txt");
				...
				int segmentId = 0;
				var forceTriSegmentIds = new List<int>();
				for (int ipolygon = npolygons - 1; ipolygon != -1; ipolygon--) {
					...
					for (int iloop = eloops.Count - 1; iloop != -1; iloop--) {
						...
						var end = count;
						ulong lastValue = edges[0].Edge.UserValue;
						int segmentStart = -1;
						for (int i = 1; i <= end; i++) {
							var j = i % count;
							var euv = edges[j].Edge.UserValue;
							if (euv != lastValue) {
								if (segmentStart == -1) {
									...
									if ((segmentCode & (1UL << 63)) == 0) {
										...
										for (int k = segmentStart; k < i; k++)
											edges[k % count].Edge.UserValue = segmentCode;
									}
						...
						if (segmentStart == -1) {
							...
							if ((segmentCode & (1UL << 63)) == 0) {
								...
								for (int k = count - 1; k != -1; k--)
									edges[k].Edge.UserValue = segmentCode;
							}

							// セグメントコードからセグメントIDを取得し登録する
							int id;
							if (segmentIds.TryGetValue(segmentCode, out id)) {
								segments.Add(new SharedSegment(id, 0, count));
							} else {
								segments.Add(new SharedSegment(segmentId, 0, count));
								segmentIds.Add(segmentCode, segmentId);
								segmentId++;
							}
						}
			...
			Dbg.StackWrite(EPolygonsDebug(epolygons), "AfterSet.txt");
				...
				if (forceTriSegmentIds.Count != 0) {
					for (int ipolygon = npolygons - 1; ipolygon != -1; ipolygon--) {
						...
						for (int iloop = eloops.Count - 1; iloop != -1; iloop--) {
							var segments = eloops[iloop].UserData as List<SharedSegment>;
...
#if SETSEG_WRITE
			var debugSegments = new Dictionary<int, List<SharedSegment>>();
			for (int ipolygon = npolygons - 1; ipolygon != -1; ipolygon--) {
				var eloops = epolygons[ipolygon].Loops;
				for (int iloop = eloops.Count - 1; iloop != -1; iloop--) {
					var segments = eloops[iloop].UserData as List<SharedSegment>;
					for (int isegment = segments.Count - 1; isegment != -1; isegment--) {
						...
						List<SharedSegment> list;
						debugSegments.TryGetValue(s.Id, out list);
						if (list == null) {
							list = new List<SharedSegment>();
...
#if BOOL2D_LOG
	static string EPolygonsDebug(List<PolBoolF.EPolygon> epolygons) {
		...
		/// <returns>属性</returns>
		static EdgeAtr FindAtr(List<List<polbool.Polygon>> groups, polbool.Edge edge) {
			// 状況に応じて適切な属性を選択する
			// ※グループインデックスが大きい方が後から描画されたグループとなっているため、
			// ※エッジに関わったグループの内インデックスが最大のものから属性を取得する
		...
		/// <typeparam name="T">頂点型</typeparam>
		static void EnableVertices<T>(List<T> vertices, int segmentStart, int segmentEnd, bool[] segmentEnables, Func<T, vector> project, element threshould, bool forceTri) {
			var stack = new Stack<VertexSegment>();
			var vcount = vertices.Count; // ポリゴンの頂点数
			...
			if (segmentStart % vcount == segmentEnd % vcount) {
				// セグメントがループ状の場合
				...
				for (int i = vcount - 1; i != -1; i--) {
					var x = axisX.Dot(project(vertices[i]) - c);
					if (x < maxDistance) {
						maxDistance = x;
						loopEnd = i;
					}
				...
				stack.Push(new VertexSegment(loopEnd, loopStart + vcount, 0));
			...
			while (stack.Count != 0) {
				...
				var o = project(vertices[seg.Start % vcount]);
				var axisX = (project(vertices[seg.End % vcount]) - o).Normalize();
				var axisY = new vector(-axisX.Y, axisX.X);
				element maxY = element.MinValue;
				int index = -1;
				for (int i = seg.End - 1; seg.Start < i; i--) {
					var y = Math.Abs(axisY.Dot(project(vertices[i % vcount]) - o));
					if (maxY < y) {
						maxY = y;
						index = i;
					}
				...
				if (maxY <= threshould) {
					...
					if (seg.Depth == 0 && forceTri && 0 <= index) {
						segmentEnables[(index - segmentStart) % vcount] = true;
					}
					continue;
				}
		...
		/// <typeparam name="T">頂点型</typeparam>
		static void ExtractEnabledVertices<T>(List<T> vertices, bool[] enables, List<T> reducedVertices) {
