...
namespace Jk {
	...
	[Serializable]
	public struct Range4i : IJsonable {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range4i(vector min, vector max, bool normalize) {
			if (normalize) {
		...
		public Range4i(IEnumerable<vector> positions) {
		...
		public Range4i(IEnumerable<volume> volumes) {
			vector min = vector.MaxValue, max = vector.MinValue;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range4i(Range4f v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Range4i(Range4d v) {
			Min = new vector(v.Min);
			Max = new vector(v.Max);
		}
		...
		public bool Intersects(volume range) {
			...
			if (Max.W < range.Min.W || range.Max.W < Min.W) return false;
			return true;
		}
